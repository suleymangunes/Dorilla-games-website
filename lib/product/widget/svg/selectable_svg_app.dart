...
class SelectableSvgApp extends StatelessWidget {
  const SelectableSvgApp({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<IsSelectedApp, bool>(
      builder: (context, state) {
        return InkWell(
          onTap: () {
            context.read<IsSelectedApp>().select();
          },
          child: Column(
            children: [
              Card(
                ...
                child: SmallSvg(svg: SvgEnum.app.getSvg),
              ),
              const SmallTextAllCard(text: ProjectKeys.app),
            ],
          ),
        );
      },
